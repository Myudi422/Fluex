...
Padding(
  ...
  child: TextField(
    ...
    style: TextStyle(color: Colors.white), // Tambahkan ini untuk mengubah warna teks
    decoration: InputDecoration(
      hintText: 'Cari episode...',
      hintStyle: TextStyle(color: Colors.white.withOpacity(0.5)), // Tambahkan ini untuk mengubah warna teks petunjuk
      prefixIcon: Icon(Icons.search, color: Colors.white), // Tambahkan ini untuk mengubah warna ikon pencarian
      suffixIcon: isSearching
          ? IconButton(
              icon: Icon(Icons.clear, color: Colors.white), // Tambahkan ini untuk mengubah warna ikon hapus
              onPressed: () {
                setState(() {
        ...
        child: Padding(
          ...
          child: SingleChildScrollView(
            ...
            child: GridView.builder(
              ...
              itemCount: isSearching ? filteredEpisodes.length : episodes.length,
...
Widget _buildEpisodeItem(
    BuildContext context, int index, bool isSearching) {
  ...
  String imageUrl = episode['link_gambar'] ?? ''; // Assuming 'link_gambar' is the image URL property
  ...
  // Filter episode based on search
  final bool isMatchSearch = episode['episode_number']
      .toString()
      .contains(searchController.text.toLowerCase());

  if (!isMatchSearch) {
    return Container(); // Hide the item if it doesn't match the search
  }
